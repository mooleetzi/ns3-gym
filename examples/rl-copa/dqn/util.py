...
def plot_learning_curve(score_list, avg_score_list, env,dir):
    ...
    plt.savefig('{}/{}_dqn_learning_curve.png'.format(dir, env))
    plt.close()


def store_training_data(episodes, scores, avg_scores, epsilons, env, dir):
    training_data_dict = {"episode": episodes, "score": scores, "avg_score": avg_scores, "epsilon": epsilons}
    df = pd.DataFrame(training_data_dict)
    df.to_csv("{}/training/{}_dqn_training_data.csv".format(dir,env))


def store_training_config(config, env, dir):
    df = pd.DataFrame(config, index=[0])
    df.to_csv("{}/training/{}_dqn_training_config.csv".format(dir,env))
