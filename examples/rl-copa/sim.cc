...
NS_LOG_COMPONENT_DEFINE ("TcpVariantsComparisonCopa");
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("envTimeStep", "Time step interval for time-based TCP env [s]. Default: 0.1s",
                tcpEnvTimeStep);
  // other parameters
  ...
  if (transport_prot.compare ("ns3::TcpCopaRl") == 0)
    {
  ...
  //
  //   system (("cp -R plot-scripts/cwnd.plt " + dir).c_str ());
  system (("cp -R plot-scripts/throughput.plt " + dir).c_str ());
  ...
  PrintRxCount ();
  Simulator::Destroy ();
  system (("cd " + dir + " && gnuplot throughput.plt")
              .c_str ());
  return 0;
}
